...
function initHeader() {
    ...
    for (var i in points) {
        var c = new Circle(points[i], 3 + Math.random() * 2, 'red');
        points[i].circle = c;
    }
}
...
function drawLines(p) {
    ...
    for (var i in p.closest) {
        ...
        ctx.strokeStyle = 'rgba(0,233,118,' + p.active + ')';
        ctx.stroke();
    }
}
...
function Circle(pos, rad, color) {
    ...
    this.draw = function() {
        ...
        ctx.fillStyle = 'rgba(0,233,118,' + _this.active + ')';
        ctx.fill();
    };
}
...
(function(window) {
    ...
    function createHome() {
        ...
        homeDiv.innerHTML = `<div class="home_container">
                                    <h2>I am G<span style="position: relative; top: -0.4em;">M</span>L</h2>
                                    <p>
                                    <br>A Graph Manipulation Language<br>
                                    <br><b>THE BURNT GARLIC</b>
                                    </p>
                                    <ul style='font-size: 20px;'><i>
                                    <li> Subham Ghosh : 20CS10065
                                    <li> Anubhav Dhar : 20CS30004
                                    <li> Aritra Mitra : 20CS30006</i>
                                    </ul>
                                    <div class="close_home" href="">x</div></div>`;
    ...
    $(search_form).submit(function(event) {
        ...
        if ('about' === $("input").val()) {
            ...
            fdata.append('file', document.getElementById('graph__file').files[0]);
            fdata.append('command', binder);
            ...
            // xhr.setRequestHeader('boundary', '')
            xhr.onload = function() {
                if (xhr.status === 200) {
                    let response = JSON.parse(xhr.responseText);
                    // console.log(response);
                    var outp = document.createElement('pre');
                    outp.innerHTML = response.output;
                    outp.setAttribute('class','fridge');
                    // outp.style.scrollbarWidth = 'thin';
                    $(outp).appendTo(terminal_div);
                    const element = document.getElementById("search__input");
                    element.scrollIntoView({ behavior: "smooth" });

                } else {
                    console.error('Request failed.  Returned status of ' + xhr.status);
                    alert("Some error occured while processing your query!");
                    const element = document.getElementById("search__input");
                    element.scrollIntoView({ behavior: "smooth" });
                }
            };
